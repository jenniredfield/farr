...
export function Header() {
  return (
    <header className="sticky top-0 z-50 border-b border-secondary-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        ...
        <nav className="hidden gap-8 text-sm font-medium text-secondary-600 sm:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
